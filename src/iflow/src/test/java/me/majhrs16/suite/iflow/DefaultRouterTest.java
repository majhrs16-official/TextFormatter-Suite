...
class DefaultRouterTest {
    ...
    void defaultPolicyAcceptsWhenNothingConfigured() {
        ...
        RouteOutcome outcome = router.route(chat(STEVE, "chat"), ALEX);
    ...
    void firstMatchingRuleWinsByPriority() {
        ...
        assertEquals(PolicyTarget.LOG, router.route(chat(STEVE, "chat"), ALEX).decision().target());
    }
    ...
    void dropRuleDiscardsForMatchedChannelOnly() {
        ...
        assertEquals(PolicyTarget.DROP, router.route(chat(STEVE, "private"), ALEX).decision().target());
        assertEquals(PolicyTarget.LOG, router.route(chat(STEVE, "chat"), ALEX).decision().target());
    }
    ...
    void rateLimitRuleBacksOffAfterBudget() {
        ...
        assertEquals(PolicyTarget.LOG, router.route(chat(STEVE, "chat"), ALEX).decision().target());
        RouteOutcome throttled = router.route(chat(STEVE, "chat"), ALEX);
        assertEquals(PolicyTarget.RATE_LIMIT, throttled.decision().target());
        assertEquals(true, throttled.decision().backoffMillis() > 0);
    }
    ...
    void emitSeesOwnMessageWithoutExtraPermission() {
        ...
        RouteOutcome toSelf = router.route(chat(STEVE, "chat"), STEVE);
        assertEquals(PolicyTarget.LOG, toSelf.decision().target());
    }
}