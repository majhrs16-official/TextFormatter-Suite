 ...
 * format templates.
 *
 * <p>This is the atomic "arsenal" container: {@code texts[0]} is translated
 * using {@code formats[0]}, {@code texts[1]} with {@code formats[1]}, and so
 * on. An empty formats array means the texts are used literally (the original
 * engine's "variable literal" behaviour). Multiple messages are naturally
 * represented by multiple entries, not by a fixed-size from/to pair.</p>
 */
