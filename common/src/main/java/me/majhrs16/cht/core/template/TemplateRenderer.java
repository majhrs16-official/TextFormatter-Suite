...
import me.majhrs16.cht.core.scripting.ExpressionEvaluationException;
 ...
 *   <li>optional {@code <expr>} scripting tags are evaluated (CoT);</li>
 ...
 * </ol>
 *
 * <p>The final step is the only place MiniMessage markup is interpreted, so
 * anything taken from players is already escaped and cannot inject tags. A
 * {@code <tr>} span does not nest; the first {@code </tr>} closes it.</p>
 */
public final class TemplateRenderer {
    ...
    private String replaceBuiltins(String source, TemplateContext context) {
        ...
        result = result.replace("%player_name%",
            MiniEscape.escape(context.sender().name()));
        result = result.replace("%player_uuid%",
            MiniEscape.escape(safeUuid(context.sender())));
        result = result.replace("%lang_source%",
            MiniEscape.escape(context.sourceLanguage().code()));
        result = result.replace("%lang_target%",
            MiniEscape.escape(context.targetLanguage().code()));
        for (String key : context.variables().keySet()) {
            result = result.replace("%" + key + "%",
                MiniEscape.escape(context.variable(key)));
        }
        return result;
    }
    ...
    private String resolveExternalTokens(String source, TemplateContext context) {
        ...
        while (matcher.find()) {
            ...
            if (isContentToken(matcher.group())) {
                continue;
            }
    ...
    /**
     * Translates the raw content of a span. Dynamic values inside the span
     * were already escaped at insertion time (HTML entities), so the result is
     * safe to embed into the final MiniMessage template as-is.
     */
    private String translateSpan(String content, TemplateContext context) {
        ...
        if (!translation.isAvailable()) {
            logger.debug("Translation unavailable, leaving <tr> span untouched");
            return content;
        }
    ...
    private static List<Span> findSpans(String source) {
        ...
        while (index < source.length()) {
            ...
            if (close < 0) {
                break; // unterminated span: leave the rest as literal text
            }
    ...
    private static String safeUuid(Subject subject) {
        return subject.uuid() != null ? subject.uuid().toString() : "";
    }
