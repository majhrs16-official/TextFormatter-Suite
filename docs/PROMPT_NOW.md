...
|---|-------|-----------|-----------------|
| A1 | **Persistencia de idioma** + `/suite lang [jugador] <code\|auto\|off>` | **P0** | Port `UserLanguageStore` en core-api/spi (load/save uuid→Language/AUTO/OFF); backend YAML primero (`storage.yml`), SQLite/MySQL después. spigot-host: Actor.language = store→locale→null. OFF = sin traducción. |
| A2 | **Eventos no-chat en spigot-host**: join/quit/death (+advancement) | **P0** | MessageType ya existe. Convención de canales: `join`, `quit`, `death`; si el canal no existe en registry → evento silencioso (presencia = configuración). Dirección: ALL + espejo consola vía regla REDIRECT, no hardcode. |
| A3 | **Claim-mode configurable**: `cancel-event` \| `clear-recipients` | **P0** | Clave nueva en config.yml (ej. `chat.claim-mode`), HostConfig+ConfigLoader+defaults embebidos+editor paths (schema v2.3?). clear-recipients: no cancelar el event; vaciar recipients para que vanilla solo procese al sender. |
| A4 | PlaceholderResolver adapter PAPI en spigot-host | P1 | ~30 líneas; pasar a TextFormatters.create vía SuiteHost.bootstrap (hoy bootstrap pasa null → añadir overload). |
| A5 | Comandos paridad: lang(A1)/toggle/reset/status | P1 | toggle per-jugador usa OFF del store. |
| A6 | Self-traducción UI: strings en lang/ | P1 | Recuperar el 98% del original; los textos del plugin pasan por el MISMO pipeline de traducción. |
